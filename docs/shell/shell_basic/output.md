...
exec 2>testerror
echo "This is a test start of the script"
...
$ 
$ ./test11 
This is a test start of the script
...
在脚本中重定向输入和输出时，并不局限于这 3 个默认的文件描述符。在 shell 中最多可以有 9 个打开的文件描述符。其他 6 个从 3~8 的文件描述符均可用作输入或输出重定向。你可以将这些文件描述符中的任意一个分配给文件，然后在脚本中使用它们。本节将介绍如何在脚本中使用其他文件描述符。
...
$ 
$ ./badtest 
./badtest: line 7: 3: Bad file descriptor
$ 
$ cat test17file 
...
bash    2063 qinghuo    0u   CHR  136,0      0t0    3 /dev/pts/0
bash    2063 qinghuo    1u   CHR  136,0      0t0    3 /dev/pts/0
bash    2063 qinghuo    2u   CHR  136,0      0t0    3 /dev/pts/0
$
```

上例显示了当前进程（bash shell）的默认文件描述符（0、1 和 2）。同时可以注意到，因为是以非 root 用户执行，没有权限查看全部文件描述符信息，lsof 提示了你信息可能不完整(和教材不同，这里并未输出警告信息)。lsof 的默认输出中有 7 列信息，详情如下。
...
$ ./t18 
COMMAND  PID    USER   FD   TYPE DEVICE SIZE/OFF    NODE NAME
t18     5070 qinghuo    0u   CHR  136,0      0t0       3 /dev/pts/0
t18     5070 qinghuo    1u   CHR  136,0      0t0       3 /dev/pts/0
t18     5070 qinghuo    2u   CHR  136,0      0t0       3 /dev/pts/0
t18     5070 qinghuo    3w   REG  259,3        0 4340649 /home/qinghuo/Desktop/shell/output/test18file1
t18     5070 qinghuo    6w   REG  259,3        0 4340650 /home/qinghuo/Desktop/shell/output/test18file2
t18     5070 qinghuo    7r   REG  259,3       73 4338134 /home/qinghuo/Desktop/shell/output/testfile
$ 
```
...
```bash
$ ll /dev/null 
crw-rw-rw- 1 root root 1, 3 Jan  6 11:38 /dev/null
$ cat /dev/null 
$
```
...
```bash
$ ll badfile t16 2> /dev/null
-rwxr-xr-x 1 qinghuo qinghuo 136 Jan  6 12:37 t16*
$ 
```
...
testing.qUKJ4X
$ ll | grep testi
-rw------- 1 qinghuo qinghuo    0 Jan  6 13:15 testing.qUKJ4X 
$
```
...
/tmp/test.35yhki
$ ll /tmp/test*
-rw------- 1 qinghuo qinghuo 0 Jan  6 13:23 /tmp/test.35yhki
$ 
```
...
$ 
$ ./t20
The temp file is located at: /tmp/tmp.1SVWjW
This is a test file.
...
$ 
$ ./t21
Sending data to directory dir.9WR8LV
$
$ ll | grep dir
drwx------ 2 qinghuo qinghuo 4096 Jan  6 13:29 dir.9WR8LV/
$ cd dir.9WR8LV/
$ ls -al
total 16
drwx------ 2 qinghuo qinghuo 4096 Jan  6 13:29 .
drwxr-xr-x 3 qinghuo qinghuo 4096 Jan  6 13:29 ..
-rw------- 1 qinghuo qinghuo   44 Jan  6 13:29 temp.3pMQ0d
-rw------- 1 qinghuo qinghuo   44 Jan  6 13:29 temp.KlSPLA
$ cat temp.3pMQ0d 
This is a test line of data for temp.3pMQ0d
$ cat temp.KlSPLA 
This is a test line of data for temp.KlSPLA
$ 
```
...
```bash
$ date | tee testfile
Fri Jan  6 01:42:39 PM CST 2023
$ cat testfile 
Fri Jan  6 01:42:39 PM CST 2023
$ 
```
...
```bash
$ who | tee testfile
qinghuo  tty1         2023-01-06 11:38
qinghuo  pts/2        2023-01-06 11:55 (tmux(2147).%0)
qinghuo  pts/3        2023-01-06 11:55 (tmux(2147).%1)
$ cat testfile 
qinghuo  tty1         2023-01-06 11:38
qinghuo  pts/2        2023-01-06 11:55 (tmux(2147).%0)
qinghuo  pts/3        2023-01-06 11:55 (tmux(2147).%1)
